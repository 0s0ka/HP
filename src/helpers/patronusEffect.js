...
const QUANTITY = 25; //количество световых точек
...
export function init() {
  ...
  if (canvas && canvas.getContext) {
    ...
    window.addEventListener("mousedown", documentMouseDownHandler);
...
function documentMouseDownHandler(event) {
  if (event) mouseIsDown = true;
}
function documentMouseUpHandler(event) {
  if (event) mouseIsDown = false;
}
